...
def _expr_str(node: ast.expr) -> str:
    match node:
        case ast.Constant(value=v):
            ...
            if isinstance(v, str):   return f"/* TODO: string literal */ estus__packn()"
        ...
        case ast.BinOp(left=l, op=op, right=r):
            # TODO: replace with duck arithmetic ops once defined in duckbox.c
            # Raw C ops on uint64_t ducks corrupt NaN-boxed values.
            c_op = _BIN_OPS.get(type(op))
            if c_op:
                return f"({_expr_str(l)} {c_op} {_expr_str(r)})"
            if isinstance(op, ast.Pow):
                return f"/* TODO: duck pow({_expr_str(l)}, {_expr_str(r)}) */"
            if isinstance(op, ast.FloorDiv):
                return f"/* TODO: duck floordiv({_expr_str(l)}, {_expr_str(r)}) */"
            return f"/* TODO: BinOp {type(op).__name__} */"
        ...
        case ast.UnaryOp(op=op, operand=o):
            match op:
                ...
                case ast.Invert():
                    return f"/* TODO: duck bitwise not (~{_expr_str(o)}) */"
                case ast.UAdd():
                    return _expr_str(o)
                case _:
        ...
        case ast.BoolOp(op=op, values=values):
            # TODO: Python's and/or return one of their operands, not a bool.
            # This emits a packed bool instead — correct for conditionals, wrong for assignment.
            c_op = "&&" if isinstance(op, ast.And) else "||"
            parts = f" {c_op} ".join(f"estus__unpack_truthy({_expr_str(v)})" for v in values)
            return f"estus__packb({parts})"

        case ast.Compare(left=left, ops=ops, comparators=comparators):
            if len(ops) == 1:
                c_op = _CMP_OPS.get(type(ops[0]))
                if c_op:
                    # TODO: duck comparison ops (raw compare on uint64_t is wrong for floats/ints)
        ...
        case ast.Subscript(value=obj, slice=idx):
            # TODO: proper container index ops via runtime
            if isinstance(idx, ast.Constant) and isinstance(idx.value, int):
                return f"{_expr_str(obj)}[{idx.value}]"
            return f"{_expr_str(obj)}[estus__unpacki({_expr_str(idx)})]"
...
def _compile_stmts(stmts: Sequence[ast.stmt], c_lines: list[str]) -> None:
    for stmt in stmts:
        match stmt:
            case ast.AnnAssign():  _ann_assign(stmt, c_lines)
            case ast.Assign():     _assign(stmt, c_lines)
            case ast.AugAssign():  _aug_assign(stmt, c_lines)
            case ast.If():         _if_stmt(stmt, c_lines)
            case ast.While():      _while_stmt(stmt, c_lines)
            case ast.For():        _for_stmt(stmt, c_lines)
...
def _ann_assign(node: ast.AnnAssign, c_lines: list[str]) -> None:
...
def _assign(node: ast.Assign, c_lines: list[str]) -> None:
    value = _expr_str(node.value)
    for target in node.targets:
        c_lines.append(f"{_expr_str(target)} = {value};")  # type: ignore[arg-type]


def _aug_assign(node: ast.AugAssign, c_lines: list[str]) -> None:
    # TODO: replace with duck arithmetic ops once defined
    target = _expr_str(node.target)  # type: ignore[arg-type]
    c_op = _BIN_OPS.get(type(node.op), f"/* {type(node.op).__name__} */")
    c_lines.append(f"{target} = ({target} {c_op} {_expr_str(node.value)});")


def _if_stmt(node: ast.If, c_lines: list[str]) -> None:
    c_lines.append(f"if (estus__unpack_truthy({_expr_str(node.test)})) {{")
    _compile_stmts(node.body, c_lines)
    c_lines.append("}")
    while node.orelse:
        if len(node.orelse) == 1 and isinstance(node.orelse[0], ast.If):
            ...
            c_lines.append("else {")
            _compile_stmts(node.orelse, c_lines)
            c_lines.append("}")
            break


def _while_stmt(node: ast.While, c_lines: list[str]) -> None:
    c_lines.append(f"while (estus__unpack_truthy({_expr_str(node.test)})) {{")
    _compile_stmts(node.body, c_lines)
    c_lines.append("}")


def _for_stmt(node: ast.For, c_lines: list[str]) -> None:
...
def compile(tree: ast.Module) -> str:
    ...
    for i, line in enumerate(script):
        opens  = line.count("{")
        closes = line.count("}")
        script[i] = "\t" * max(depth - closes, 0) + line
        depth += opens - closes
